...
def is_friday(date_str):
    ...
    return dt.weekday() == 4  # Friday is 4 (Monday is 0)
...
def backtest_simulation(df, initial_capital=2000, max_dd=0.15, daily_limit=0.05):
    ...
    for i in range(len(df)):
        ...
        if is_friday(current_date) or is_economic_event_day(current_date, df):
        ...
        if current_position == 'LONG':
            current_equity = entry_capital * (1 + daily_change_pct)
        elif current_position == 'SHORT':
            current_equity = entry_capital * (1 - daily_change_pct)
        
        # Check drawdown and daily limits
        ...
        if current_position is not None:
            daily_return = daily_change_pct if current_position == 'LONG' else -daily_change_pct
            if daily_return < -daily_limit:
                # Force exit due to daily limit
                capital = entry_capital * (1 + daily_return)
                if current_position == 'LONG':
        ...
        if current_position is None:
            ...
            if buy_signals >= 2:  # Lowered from 3 to 2 for more entries
                # Enter long position
                position_size = capital  # Use full available capital
            ...
            elif sell_signals >= 2:  # Lowered from 3 to 2 for more entries
            ...
            if current_position == 'LONG':
                ...
                if sell_signals >= 3:  # Lowered from 4 to 3 for more exits
                    # Exit long position
                    capital = entry_capital * (1 + daily_change_pct)
                    ...
                    # Enter short if still strong sell signals
                    if sell_signals >= 3:
                        current_position = 'SHORT'
                        entry_price = current_price
                        entry_capital = capital
                ...
                if buy_signals >= 3:  # Lowered from 4 to 3 for more exits
                    # Exit short position
                    capital = entry_capital * (1 - daily_change_pct)
                    return_pct = -daily_change_pct
                    if return_pct > 0:
                    ...
                    # Enter long if still strong buy signals
                    if buy_signals >= 3:
                        current_position = 'LONG'
                        entry_price = current_price
                        entry_capital = capital
...
def generate_report_md(results, output_file='backtest_report.md'):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("\n## Key Observations\n")
        f.write("1. The strategy survived 18 years of market cycles including 2008 financial crisis, 2020 COVID crash, and various bull/bear markets.\n")
        f.write("2. The combination of astro-quant signals with strict risk management proved resilient.\n")
        f.write("3. The 15% maximum drawdown constraint was effective in protecting capital during volatile periods.\n")
        ...
        f.write("\n## Signal Logic\n")
        f.write("### BUY Signals (3+ needed for entry):\n")
        f.write("- Moon in Leo\n")
        ...
        f.write("### SELL Signals (3+ needed for entry):\n")
        ...
        f.write("favorable astro-quant alignments.")
