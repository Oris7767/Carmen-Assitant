...
def backtest_simulation(df, initial_capital=2000, max_dd=0.15, daily_limit=0.05):
    ...
    total_capital = initial_capital  # Total portfolio value
    equity_curve = [initial_capital]
    current_position = None  # 'LONG' or 'SHORT'
    position_size = 0  # Amount allocated to current position
    entry_price = 0
    total_trades = 0
    winning_trades = 0
    losing_trades = 0
    ...
    for i in range(len(df)):
        ...
        if current_position == 'LONG':
            current_position_value = position_size * (1 + daily_change_pct)
        ...
        # Check drawdown
        dd = (max_equity - total_capital) / max_equity if max_equity > 0 else 0
        if dd > max_dd:
            print(f"Drawdown limit exceeded on {current_date.date()}, stopping...")
            break
            
        # Update max equity
        if total_capital > max_equity:
        ...
        if current_position is not None:
            ...
            if daily_return < -daily_limit:
                ...
                if current_position == 'LONG':
                    cash += position_size * (1 + daily_change_pct)
        ...
        if current_position is None:
            ...
            if buy_signals >= 2:  # At least 2 strong signals for entry
                # Enter long position using a portion of cash to manage risk
                position_size = cash * 0.02  # Use 2% of available cash to manage risk
                cash -= position_size  # Deduct from available cash
                ...
                trade_log.append({
                    ...
                    'action': 'ENTRY LONG',
                    'price': entry_price,
                    'capital': cash + position_size,  # Total portfolio value
                    'signals': buy_signals
                })
            elif sell_signals >= 2:  # At least 2 strong signals for short entry
                # Enter short position using a portion of cash to manage risk
                position_size = cash * 0.02  # Use 2% of available cash to manage risk
                cash -= position_size  # Deduct from available cash
            ...
            if current_position == 'LONG':
                ...
                if sell_signals >= 3:  # Stronger signal needed to reverse
                    # Exit long position
                    cash += position_size * (1 + daily_change_pct)  # Add gains/losses to cash
                    return_pct = daily_change_pct
                    total_trades += 1
                    if return_pct > 0:
                        winning_trades += 1
        ...
        # Update total capital after any transaction
        if current_position == 'LONG':
            total_capital = cash + position_size * (1 + daily_change_pct)
        elif current_position == 'SHORT':
            total_capital = cash + position_size * (1 - daily_change_pct)
        else:
            total_capital = cash
...
def generate_report_md(results, output_file='backtest_report.md'):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("- Position Size: 2% of available cash per trade\n\n")
        ...
        for i, trade in enumerate(results['trade_log'][:20]):
            ...
            return_pct = f"{trade.get('return_pct', 0)*100:.2f}%" if 'return_pct' in trade and trade['return_pct'] is not None else "N/A"
