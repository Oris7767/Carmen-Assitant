...
async function createDraftPost(page, opts) {
  ...
  for (const sel of bodySelectors) {
    try {
      ...
      if (el) {
        ...
        // Dispatch a synthetic paste event with clipboard data
        // This works reliably in headless mode across platforms (no system clipboard needed)
        await el.click();
        await page.waitForTimeout(500);
  ...
  if (!bodySet) {
    log('⚠️  Body field not found — trying keyboard navigation + paste event');
  ...
  // Look for Save/Publish buttons (EN + FR)
  const saveSelectors = [
  ...
  // Strategy 1: Look for a "Save" or "Save draft" or French equivalent button
  ...
  if (!saved) {
    // Strategy 2: Click Publish dropdown and select "Save as draft" / "Brouillon"
    for (const sel of publishSelectors) {
      try {
        ...
        if (btn && await btn.isVisible()) {
          ...
          const dropdown = await page.$('button[aria-label*="more" i], [data-testid="publish-dropdown"], button[aria-label*="plus" i]');
          if (dropdown) {
            await dropdown.click();
            await page.waitForTimeout(500);
            const draftOption = await page.$('button:has-text("Save as draft"), button:has-text("Brouillon"), button:has-text("brouillon"), div:has-text("Save as draft"), div:has-text("Brouillon"), [role="menuitem"]:has-text("draft"), [role="menuitem"]:has-text("brouillon")');
            if (draftOption) {
  ...
  if (!saved) {
    ...
    // Patreon auto-saves — check specific save status indicator, not just body text
    try {
      await page.waitForFunction(() => {
        // Look for specific save status text near the X close button (top-left corner)
        const topBar = document.querySelector('header') || document.querySelector('[role="banner"]') || document.body;
        const headerText = topBar.textContent || '';
        // Check for "Saved" indicators specifically in the header area
        const statusPatterns = /Enregistrée|Saved|Brouillon|Enregistré/i;
        return statusPatterns.test(headerText);
      }, { timeout: 25000 });
      await page.waitForTimeout(2000);
      log('✅ Autosave confirmed');
    } catch (e) {
      log('⚠️  Save status not confirmed in header — waiting extra time');
      await page.waitForTimeout(10000);
    }
  }
  
  // Extra wait for save to fully commit
  await page.waitForTimeout(3000);
